...
export const PerformanceDashboard: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} id="performance" className="reveal py-24 bg-[var(--white-smoke)]">
      <div className="mx-auto max-w-6xl px-6">
        <div className="text-center max-w-2xl mx-auto">
          <span className="kicker text-gradient-gold">Tableau de bord</span>
          <h2 className="luxury-heading mt-3">Performance du portefeuille</h2>
          <p className="luxury-subheading mt-5 pt-8">Données simulées à titre indicatif. Les performances passées ne préjugent pas des performances futures.</p>
        </div>
        ...
        <div ref={gridRef} className="reveal-stagger grid grid-cols-1 lg:grid-cols-3 gap-6 mt-12">
          ...
          <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-4">
            {kpis.map((k, i) => (
              <div key={i} className="stat-card card-hover">
                <div className="text-secondary text-xs mb-1">{k.label}</div>
          ...
          <div className="stat-card">
            <div className="font-display mb-3">Allocation d'actifs</div>
            <div className="space-y-3">
              {allocation.map((a, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-24 h-2 rounded bg-[var(--white-smoke)] overflow-hidden">
                    <div
                      className="h-full bg-[var(--gold-metallic)]"
                      style={{ width: `${a.value}%` }}
                    />
                  </div>
                  <div className="text-sm text-secondary flex-1">{a.label}</div>
                  <div className="font-display text-sm">{a.value}%</div>
                </div>
              ))}
            </div>
            <div className="mt-4">
              <div className="text-xs text-secondary">Principales lignes</div>
              <ul className="mt-2 space-y-1">
                {topHoldings.map((h, i) => (
                  <li key={i} className="flex justify-between text-sm">
                    <span className="text-secondary">{h.name}</span>
                    <span className="font-display">{h.weight}%</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="stat-card lg:col-span-2">
            <div className="flex items-center justify-between mb-3">
              <div className="font-display">Portefeuille vs BRVM</div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                      range === r
                        ...
                        : "border border-[var(--night)]/10 text-secondary hover:bg-[var(--white-smoke)]"
                    }`}
              ...
              <svg viewBox="0 0 540 180" className="w-full h-48">
                <defs>
                  <linearGradient id="gradPortfolio" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--gold-light)" stopOpacity="0.8" />
                    <stop offset="100%" stopColor="var(--gold-metallic)" stopOpacity="0.2" />
                  </linearGradient>
                ...
                {[...Array(6)].map((_, i) => (
                  <line key={i} x1="0" x2="540" y1={i * 30} y2={i * 30} stroke="var(--night-10)" />
                ))}

                {/* Benchmark */}
                <path d={linePath("benchmark")} fill="none" stroke="url(#gradBenchmark)" strokeWidth="2" />
                {/* Portfolio */}
                <path d={linePath("portfolio")} fill="none" stroke="url(#gradPortfolio)" strokeWidth="3" />
              </svg>
            </div>
            <div className="mt-2 flex items-center gap-4 text-xs text-secondary">
              <div className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-full" style={{ background: "var(--gold-metallic)" }} /> Portefeuille</div>
              <div className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-full bg-slate-400" /> BRVM</div>
            </div>
          </div>

          {/* Notes */}
          <div className="stat-card">
            <div className="font-display mb-2">Notes méthodologiques</div>
            <ul className="text-sm text-secondary list-disc pl-5 space-y-2">
              <li>Données simulées pour démonstration visuelle uniquement.</li>
              <li>Benchmark utilisé: indice composite BRVM (approximation).</li>
              <li>Les rendements sont exprimés en %; base 100 au début de période.</li>
            </ul>
          </div>
        </div>
      </div>
