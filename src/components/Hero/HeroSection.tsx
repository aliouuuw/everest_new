...
const MountainWireframe = lazy(() =>
  import('./MountainWireframe').then((m) => ({ default: m.MountainWireframe })),
)
...
export const HeroSection: React.FC = () => {
  ...
  useEffect(() => {
    // Lenis → ScrollTrigger sync is handled globally by LenisWrapper.
    // Do NOT use scrollerProxy(document.body) here — it causes GSAP's
    // _getComputedProperty to enter infinite recursion.
    if (lenis && isReady) {
    ...
    >
      ...
      <div className="relative z-10 flex min-h-screen justify-center items-center">
        <div className="text-center px-6 max-w-3xl mx-auto">
          <span
            ref={kickerRef}
            className="kicker opacity-70 text-gradient-gold"
          >
          ...
          <p ref={subheadingRef} className="luxury-subheading mb-10">
            Votre partenaire de confiance pour le courtage BRVM, les émissions
            primaires et l'ingénierie financière.
          </p>
