...
export function SiteContentPage() {
  ...
  const sections = useMemo(() => {
    return filteredEntries.reduce<Record<string, Array<RegistryEntry>>>((acc, entry) => {
      ...
      setState((prev) => {
        ...
        return {
          ...
          [entry.id]: {
            ...
            saving: false,
            error: err instanceof Error ? err.message : "Échec de la réinitialisation",
          },
        };
      });
    }
  ...
  const overrideCount = entries.filter((e) => overrides[e.id] !== undefined).length;
    ...
    <div className="space-y-6">
      <div className="border-b border-[var(--jaune-or)]/10 pb-4">
        ...
        <p className="mt-2 max-w-2xl text-base text-[var(--night-60)]">
          Textes publics (pages vitrine). Les changements sont visibles sur le site dès enregistrement. Les IDs sont
          contrôlés par le registre — seuls les champs listés peuvent être sauvegardés.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-[var(--night)]/10 bg-white/80 p-4 shadow-sm sm:flex-row sm:flex-wrap sm:items-end">
        <div className="min-w-[200px] flex-1">
          <label htmlFor="site-content-page" className="mb-1 block text-xs font-semibold uppercase tracking-wider text-[var(--night-60)]">
            Page
          </label>
          <select
        ...
        <div className="min-w-[200px] flex-1">
          <label htmlFor="site-content-filter" className="mb-1 block text-xs font-semibold uppercase tracking-wider text-[var(--night-60)]">
            Filtrer les champs
          </label>
          <input
      ...
      <div className="rounded-xl border border-[var(--jaune-or)]/20 bg-[var(--jaune-or)]/5 px-4 py-3 text-sm text-[var(--night-60)]">
        <span className="font-semibold text-[var(--night)]">{fieldCount}</span> champ(s) sur cette page ·{" "}
        <span className="font-semibold text-[var(--night)]">{overrideCount}</span> surcharge(s) en base · max.{" "}
        {MAX_VALUE_LENGTH} caractères par champ
      </div>

      {entries.length === 0 && (
        <p className="rounded-xl border border-dashed border-[var(--night)]/15 bg-[var(--white-smoke)]/50 p-6 text-sm text-[var(--night-60)]">
          Aucun champ éditable n’est enregistré pour cette page. Ajoutez des entrées dans{" "}
      ...
      {entries.length > 0 && filteredEntries.length === 0 && (
        <p className="text-sm text-[var(--night-60)]">Aucun champ ne correspond au filtre.</p>
      )}

      <div className="space-y-8">
        {Object.entries(sections).map(([section, sectionEntries]) => (
          <details key={section} open className="group rounded-2xl border border-[var(--night)]/10 bg-white/90 shadow-sm">
            ...
            <div className="space-y-4 p-4">
              {sectionEntries.map((entry) => {
                ...
                const lines = Math.min(14, Math.max(5, field.value.split("\n").length + 2));
                ...
                return (
                  ...
                  >
                    <div className="mb-2 flex flex-wrap items-start justify-between gap-2">
                      <div>
                        <label className="font-primary text-sm font-semibold text-[var(--mauve)]">{entry.label}</label>
                    ...
                    <textarea
                      ...
                      placeholder={
                        override !== undefined ? "" : "(aucune surcharge — texte du code affiché sur le site)"
                      }
                      rows={lines}
