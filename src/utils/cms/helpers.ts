...
export const getCategoryColor = (category: string): string => {
  ...
  return colors[category] || 'bg-[rgba(70,29,76,0.08)] text-[var(--night-60)]';
};
...
export const getStatusColor = (status: string): string => {
  const colors: Record<string, string> = {
    ...
    'archived': 'bg-[rgba(70,29,76,0.08)] text-[var(--night-60)]',
  };
  return colors[status] || 'bg-[rgba(70,29,76,0.08)] text-[var(--night-60)]';
};
